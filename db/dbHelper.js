...
let getHot = function () {
    let deferred = q.defer();
    //let searchValueStr = convertQueryStr(searchValue);
    let queryStr = `select * from (select title,type from (select * from hottop where type = 'movie' ORDER BY getTime desc LIMIT 50) a order by id limit 20) x UNION ALL select * from (select title,type from (select * from hottop where type = 'tv' ORDER BY getTime desc LIMIT 50) a order by id limit 20) y;`;
    // console.log(queryStr)
    pool.getConnection((err, conn) => {
        "use strict";
        if (err) deferred.reject(err);
